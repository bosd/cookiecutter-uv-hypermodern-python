...
shutil.rmtree('src/uv_hypermodern_python.egg-info', ignore_errors=True);
for f in glob.glob('src/uv_hypermodern_python/*.so'): os.remove(f);
for f in glob.glob('src/uv_hypermodern_python/*.c'): os.remove(f);
"""

nox.options.default_venv_backend = "uv"

package = "uv_hypermodern_python"
...
def tests_compiled(session: nox.Session) -> None:
    ...
    session.install("-e", ".", env={"UV_HYPERMODERN_PYTHON_COMPILE_MYPYC": "1"})
